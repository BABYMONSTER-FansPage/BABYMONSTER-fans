...
test("bundles six static languages and translates fan posts only", async () => {
  ...
  assert.match(page, /function youtubeTrackUrl/);
  assert.match(page, /youtube\.com\/results\?search_query=/);
  assert.match(page, /BABYMONSTER \$\{trackName\} official audio/);
  assert.match(page, /href=\{youtubeTrackUrl\(trackName\)\}/);
  assert.match(page, /aria-label=\{`Play \$\{trackName\} on YouTube`\}/);
  assert.match(page, /className="spotify-player w-full rounded-sm"/);
  assert.match(css, /\.spotify-player \{[^}]*height: 352px;[^}]*min-height: 352px;/);
  assert.match(page, /function AdminAccordion/);
  assert.match(page, /<AdminAccordion title="近期活動">/);
  assert.match(page, /<AdminAccordion title="團員介紹與詳細介紹">/);
  assert.match(page, /<details key=\{event\.id \?\? index\}/);
  assert.match(page, /<details key=\{album\.id \|\| index\}/);
