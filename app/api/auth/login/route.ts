...
type UserRow = { id: number; nickname: string; email: string; role: "monstiez" | "admin" | "artist"; password_hash: string | null; password_salt: string | null };

export async function POST(request: Request) {
  try {
    const { email, password } = await request.json() as Record<string, string>;
    await ensureDatabase();
    const row = await db().prepare("SELECT id, nickname, email, role, password_hash, password_salt FROM users WHERE email = ?").bind(String(email || "").trim().toLowerCase()).first<UserRow>();
    ...
    const user = { id: row.id, nickname: row.nickname, email: row.email, role: row.role };
