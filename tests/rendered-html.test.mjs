...
test("bundles six static languages and translates fan posts only", async () => {
  ...
  assert.match(i18n, /本網站為非官方粉絲站/);
  assert.match(i18n, /This is an unofficial fan site/);
  assert.match(i18n, /เว็บไซต์นี้เป็นเว็บไซต์แฟนคลับอย่างไม่เป็นทางการ/);
  assert.match(i18n, /이 웹사이트는 비공식 팬 사이트입니다/);
  assert.match(i18n, /このウェブサイトは非公式ファンサイトです/);
  assert.match(i18n, /本网站为非官方粉丝站/);
  assert.match(page, /k="footerLegalNoticeV2" fallback=\{t\.warning\}/);
