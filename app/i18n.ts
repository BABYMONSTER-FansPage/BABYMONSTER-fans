...
type FixedCopy = {
  ...
  appPurpose: string;
  heroReviewSummary: string;
  googleDataPurpose: string;
...
export const fixedMessages: Record<Locale, FixedCopy> = {
  "zh-TW": {
    ...
    fanCommunityPlatform: "非官方 BABYMONSTER 粉絲社群平台",
    ...
    heroReviewSummary: "Monstiez 是非官方 BABYMONSTER 粉絲社群，提供內容瀏覽、留言、按讚及個人資料管理；瀏覽公開內容不需登入，社群互動功能則需登入。",
    googleDataPurpose: "Google 登入僅用於登入及識別 Monstiez 帳號，本站會使用 Google 提供的基本 Profile 與 Email 資料（帳號識別碼、顯示名稱、頭像及電子郵件地址），不會要求 Gmail、Google Drive 或 Google Calendar 權限。",
    privacyPolicy: "隱私權政策",
    termsOfService: "服務條款",
  ...
  "zh-CN": {
    ...
    fanCommunityPlatform: "非官方 BABYMONSTER 粉丝社区平台",
    ...
    heroReviewSummary: "Monstiez 是非官方 BABYMONSTER 粉丝社区，提供内容浏览、留言、点赞及个人资料管理；浏览公开内容无需登录，社区互动功能则需要登录。",
    googleDataPurpose: "Google 登录仅用于登录及识别 Monstiez 帐号，本站会使用 Google 提供的基本 Profile 与 Email 数据（帐号识别码、显示名称、头像及电子邮件地址），不会请求 Gmail、Google Drive 或 Google Calendar 权限。",
    privacyPolicy: "隐私权政策",
    termsOfService: "服务条款",
  ...
  th: {
    ...
    fanCommunityPlatform: "แพลตฟอร์มชุมชนแฟน BABYMONSTER อย่างไม่เป็นทางการ",
    ...
    heroReviewSummary: "Monstiez เป็นชุมชนแฟน BABYMONSTER อย่างไม่เป็นทางการสำหรับดูเนื้อหา แสดงความคิดเห็น กดถูกใจ และจัดการโปรไฟล์ เนื้อหาสาธารณะดูได้โดยไม่ต้องเข้าสู่ระบบ ส่วนการโต้ตอบในชุมชนต้องเข้าสู่ระบบ",
    googleDataPurpose: "การเข้าสู่ระบบด้วย Google ใช้เพื่อเข้าสู่ระบบและระบุบัญชี Monstiez เท่านั้น เว็บไซต์ใช้ข้อมูล Profile และ Email พื้นฐานจาก Google ได้แก่ รหัสบัญชี ชื่อที่แสดง รูปโปรไฟล์ และอีเมล และจะไม่ขอสิทธิ์ Gmail, Google Drive หรือ Google Calendar",
  ...
  en: {
    officialAppNameLabel: "What is Monstiez?",
    fanCommunityPlatform: "Unofficial BABYMONSTER fan community platform",
    ...
    heroReviewSummary: "Monstiez is an unofficial BABYMONSTER fan community for browsing content, posting comments, liking posts, and managing profiles. Public content is available without signing in; community interactions require an account.",
    googleDataPurpose: "Google Sign-In is used only to sign in to and identify a Monstiez account. We use the basic Profile and Email data provided by Google—account identifier, display name, profile photo, and email address—and do not request access to Gmail, Google Drive, or Google Calendar.",
  ...
  ko: {
    ...
    fanCommunityPlatform: "비공식 BABYMONSTER 팬 커뮤니티 플랫폼",
    ...
    heroReviewSummary: "Monstiez는 콘텐츠 열람, 댓글, 좋아요 및 프로필 관리를 제공하는 비공식 BABYMONSTER 팬 커뮤니티입니다. 공개 콘텐츠는 로그인 없이 볼 수 있으며 커뮤니티 활동에는 로그인이 필요합니다.",
    googleDataPurpose: "Google 로그인은 Monstiez 계정 로그인 및 식별에만 사용됩니다. Google이 제공하는 기본 Profile 및 Email 정보인 계정 식별자, 표시 이름, 프로필 사진과 이메일 주소만 사용하며 Gmail, Google Drive 또는 Google Calendar 권한은 요청하지 않습니다.",
  ...
  ja: {
    ...
    fanCommunityPlatform: "非公式BABYMONSTERファンコミュニティプラットフォーム",
    ...
    heroReviewSummary: "Monstiezは、コンテンツ閲覧、コメント、いいね、プロフィール管理を提供する非公式BABYMONSTERファンコミュニティです。公開コンテンツはログイン不要で、コミュニティ機能の利用にはログインが必要です。",
    googleDataPurpose: "GoogleログインはMonstiezアカウントへのログインと識別にのみ使用します。Googleが提供する基本的なProfileおよびEmail情報（アカウント識別子、表示名、プロフィール画像、メールアドレス）のみを使用し、Gmail、Google Drive、Google Calendarへのアクセスは要求しません。",
